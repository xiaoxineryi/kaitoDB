...
impl Global{
    fn init_global_from_file(dir:String){
        ...
        let dir = dir + "/global.setting";
        let file_name = Path::new(dir.as_str());
        ...
        match File::open(file_name) {
            ...
            Err(_)=>{
                ...
                match File::create(file_name){
                    Ok(file)=>  {
                        Global::init_database_if_none(file,file_name);
                    }
                    Err(_) =>{
                        panic!("无法新建数据库");
                    }
    ...
    fn init_database_if_none(mut file:File,file_name:&Path)->File{
        file.write(&[0u8,0u8,0u8,0u8]).unwrap();
        file.write(&[0u8]).unwrap();
        file.write(file_name.display().to_string().as_bytes()).unwrap();
        file.flush().unwrap();
        file
    }
    ...
    pub unsafe fn get_and_add_oid() ->OID{
        ...
        let mut file  = OpenOptions::new()
            .write(true).open(Path::new(g.path.as_str())).unwrap();
